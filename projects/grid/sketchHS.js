...
let scl = 70; // Size of the Tiles.
let bdr = 60; // Keep border value more than scale in order to work
let mrg = 10;
let startSteps = 6;
let startSize = 10;
...
function boxs(x, y, scl, scl, steps) {
  ...
  rect(x, y, scl, scl);
  if (steps >= 0) {
    let nScl = scl * random(0.2, 1);
    // let nX = x + (scl - nScl) / 2;
    // let nY = y + (scl - nScl) / 2;
    // boxs(nX, nY, nScl, nScl, steps - 1);
    boxs(x, y, nScl, nScl, steps - 1);
  }
}
